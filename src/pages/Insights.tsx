...
import { Bug, Droplets, Activity, TrendingUp, Download, Map } from "lucide-react";
...
const sudanStatesData = [
  { name: "Khartoum", malaria: 12500, cholera: 890, dengue: 234, population: 5274321 },
  { name: "Gezira", malaria: 8900, cholera: 567, dengue: 123, population: 3575280 },
  { name: "White Nile", malaria: 7800, cholera: 445, dengue: 178, population: 2000000 },
  { name: "Blue Nile", malaria: 6700, cholera: 234, dengue: 89, population: 1200000 },
  { name: "North Kordofan", malaria: 5600, cholera: 345, dengue: 156, population: 2920000 },
  { name: "South Kordofan", malaria: 4500, cholera: 123, dengue: 67, population: 1400000 },
  { name: "Darfur", malaria: 9800, cholera: 678, dengue: 234, population: 2500000 },
];

const COLORS = ['hsl(221, 83%, 53%)', 'hsl(210, 40%, 96%)', 'hsl(220, 13%, 9%)', '#10B981', '#F59E0B'];
...
const Insights = () => {
  ...
  // KPI calculations
  const totalMalaria = sudanStatesData.reduce((sum, state) => sum + state.malaria, 0);
  const totalCholera = sudanStatesData.reduce((sum, state) => sum + state.cholera, 0);
  const totalDengue = sudanStatesData.reduce((sum, state) => sum + state.dengue, 0);
  ...
  useEffect(() => {
    ...
    map.current.on('load', () => {
      ...
      sudanStatesData.forEach((state, index) => {
        ...
        const markerSize = Math.max(10, Math.min(40, (totalStateCases / 1000) + 10));
  ...
  return (
    <div className="min-h-screen bg-background font-inter">
      ...
      <main className="container mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-muted-foreground">Malaria Cases</CardTitle>
              <Bug className="h-4 w-4 text-cool-blue" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-midnight">{totalMalaria.toLocaleString()}</div>
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-muted-foreground">Cholera Cases</CardTitle>
              <Droplets className="h-4 w-4 text-cool-blue" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-midnight">{totalCholera.toLocaleString()}</div>
              <p className="text-xs text-muted-foreground">
                <TrendingUp className="inline h-3 w-3 mr-1" />
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-muted-foreground">Dengue Cases</CardTitle>
              <Activity className="h-4 w-4 text-cool-blue" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-midnight">{totalDengue.toLocaleString()}</div>
              <p className="text-xs text-muted-foreground">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-midnight">{totalCases.toLocaleString()}</div>
              <p className="text-xs text-muted-foreground">
                Across all regions
              </p>
            </CardContent>
        ...
        <Card className="mb-8">
          <CardHeader>
            <CardTitle className="text-xl font-semibold text-midnight">Sudan Disease Heatmap</CardTitle>
          </CardHeader>
          <CardContent>
            <div 
              ref={mapContainer} 
            ...
            <p className="text-sm text-muted-foreground mt-2">
              Interactive map showing disease distribution across Sudan. Click markers for detailed information.
            </p>
          </CardContent>
        </Card>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-xl font-semibold text-midnight">Malaria Cases Trend (2019-2025)</CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-xl font-semibold text-midnight">Disease Distribution</CardTitle>
            </CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={300}>
                <PieChart>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-xl font-semibold text-midnight">Disease Cases by State</CardTitle>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={400}>
              <BarChart data={sudanStatesData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="hsl(210, 40%, 96%)" />
                ...
                />
                <Bar dataKey="malaria" stackId="a" fill="hsl(221, 83%, 53%)" name="Malaria" />
                <Bar dataKey="cholera" stackId="a" fill="hsl(210, 40%, 96%)" name="Cholera" />
                <Bar dataKey="dengue" stackId="a" fill="hsl(220, 13%, 9%)" name="Dengue" />
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
